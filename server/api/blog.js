...
const MAX_DESC_LENGTH = 200 // 説明文最大長
  ...
  blogDb.getEntryList(req.query['page'], 1/* 降順 */, (err, result) => {
    ...
    if (result.length > 0) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        // アイキャッチ画像取得
        const thumbUrl = Magic3Blog.getEyecatchImageUrl(row.be_thumb_filename, 'l', row.be_thumb_src/* 画像がない場合のデフォルト画像 */)

        // 記事概要
        let desc = ''
        if (!empty(row.be_thumb_description)) desc = row.be_description

        if (empty(desc)) {
          desc = truncate(striptags(row.be_html), MAX_DESC_LENGTH) // HTMLタグ削除、最大長設定
        }

        // 登録日時
        const date = row.be_regist_dt

        list.push({ id: row.be_id, name: row.be_name, desc: desc, thumb: thumbUrl, date: date })
      }
      // 取得データを返す
...
router.get('/gallery', [
  ...
  // DBからデータ取得
  const blogDb = new BlogDb(req.pool)
  blogDb.getImageList(req.query['page'], 1/* 降順 */, (err, result) => {
    if (err) {
      log.error('データ取得エラー コード=' + err)
      return
    }
    if (result.length > 0) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        // アイキャッチ画像取得
        const thumbUrl = Magic3Env.getResourceUrl() + row.bm_image_src
